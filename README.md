...
- **v0.11** (Marginal, P0 ✅): Recording & test export — `record start/stop/status`, `record export --format=pytest|junit5|mocha`, `record report --format=junit|html`. Remaining: `--report=allure`, `tracing-start/stop`, `video-start/stop`, `show` dashboard, `--annotate`
- **v0.12** (Marginal, ✅): VSCode extension — [`se-extension-vscode`](https://github.com/se-cli/se-extension-vscode) with MCP registration, commands, webview, status bar, Task Provider (`type: se-cli` tasks for exported mocha/pytest/junit5 test files), and **Attach to Running Browser** (`open --cdp` to drive an already-open browser). Published on the [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=se-cli.se-extension-vscode)
- **v0.13** (Core, ✅): BiDi protocol expansion (issue #76) — `preload add/remove/list` (script module), `context-new/close/list` (browser user contexts), `upload --bidi` (`input.setFiles`), `cookie-list/set --bidi --user-context` (storage partition keys), `pdf|screenshot --bidi` (`browsingContext.print`/`captureScreenshot`). 73 MCP tools. Remaining: `download-list`, BiDi-native emulation module, `setViewport`/`setBypassCSP`/`handleUserPrompt`, download events
