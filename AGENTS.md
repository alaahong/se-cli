...
| [`se-cli/se-extension-vscode`](https://github.com/se-cli/se-extension-vscode) | [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=se-cli.se-extension-vscode) | VS Code extension — MCP registration, commands, webview, status bar, Task Provider (`type: se-cli` tasks for exported test files), Attach to Running Browser (`open --cdp`), published as `se-cli.se-extension-vscode` |
...
- **Recording & export (v0.11)**: `src/recorder.ts` holds the in-memory recorder state (`createRecorder`) plus the pure renderers (`renderMochaTest`/`renderPytestTest`/`renderJunit5Test` via `renderExport`); `src/report.ts` renders JUnit XML / standalone HTML. `src/daemon/server.ts` calls `recordStep()` after every executed command while `record start` is active (never for `browser_record_*` itself); failed steps are captured but carry no codegen. `src/daemon/tools/record.ts` implements the `record start|stop|status|export|report` tools; `src/mcp-server.ts` maps the 5 `browser_record_*` MCP tools to CLI args, and `src/program.ts` passes `--browser` through only for `record export`. The recorder is per-daemon (single session), capped at 10,000 steps (oldest dropped), and lost on daemon exit. Unit tests: `tests/unit/recorder.test.ts`, `tests/unit/report.test.ts`, `tests/unit/record-tools.test.ts`; integration: `tests/integration/v0.11-record.test.ts`.
...
| 0.10.0 | `release/v0.10.0` | [#121](https://github.com/se-cli/se-cli/pull/121) | [v0.10.0](https://github.com/se-cli/se-cli/releases/tag/v0.10.0) | `@browsers-cli/se-cli@0.10.0` | 2026-08-07 |
| 0.11.0 | `release/v0.11.0` | [#124](https://github.com/se-cli/se-cli/pull/124) | [v0.11.0](https://github.com/se-cli/se-cli/releases/tag/v0.11.0) | `@browsers-cli/se-cli@0.11.0` | 2026-08-13 |
